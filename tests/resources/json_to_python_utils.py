...
def translate_domain_cases(json_filepath: str,
                           res_filepath: str,
                           verbose_test: bool = True):
    if verbose_test:
        print("Translating from {} to {}".format(json_filepath, res_filepath))
    f = load_json(json_filepath)
    ...
    save_objects(new_domain_cases, res_filepath)

    if verbose_test:
        fh = open(res_filepath, 'rb')
        logger.info("\n\n\nAll the content in the res ({}) file:".format(res_filepath))
        count = 0
        inf = ""
        while True:
            try:
...
def translate_argument_cases(json_filepath: str,
                             res_filepath: str,
                             verbose_test: bool = True):
    f = load_json(json_filepath)
    ...
    save_objects(new_argument_cases, res_filepath)
